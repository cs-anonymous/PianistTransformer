...
def apply_paper_font_settings() -> None:
    plt.rcParams.update(
        {
            "font.family": "serif",
            "font.serif": [
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("INSPIRE-AAAI2027/Figures/log_timing_deviation.png"))
    parser.add_argument("--summary", type=Path, default=Path("INSPIRE-AAAI2027/Figures/floorlog_timing_stats.json"))
    parser.add_argument("--layout", choices=("main", "supplement"), default="main")
    ...
    if args.layout == "main":
        ...
        fig, axes = plt.subplots(1, 2, figsize=(3.55, 1.45), dpi=300)
    else:
        plt.rcParams.update(
            {
                "font.size": 8,
    ...
    def dev_hist(ax, values, title):
        ...
        ax.text(0.98, 0.90, f"{inside:.1f}% in [-2, 1]", transform=ax.transAxes, ha="right", va="top")
        ax.set_xlim(-4.0, 4.0)
        ax.set_title(title)
    ...
    if args.layout == "main":
        dev_hist(axes[0], arrays["ioi_dev"], "IOI deviation (score IOI > 0)")
        dev_hist(axes[1], arrays["dur_dev"], "Duration deviation")
        axes_iterable = axes
        fig.tight_layout(pad=0.20, w_pad=0.85)
    else:
        ...
        dev_hist(axes[1, 0], arrays["ioi_dev"], "IOI deviation (score IOI > 0)")
        dev_hist(axes[1, 1], arrays["dur_dev"], "Duration deviation")
