"""
CORRECT PT Evaluation: Multi-GPU generation using manual GPU assignment
Each GPU processes its own subset of samples independently.
"""
...
def create_score_input(pitch_ids, config):
    ...
    return [int(config.pitch_start + pitch) for pitch in pitch_ids]
...
def process_gpu_worker(gpu_id, sample_indices, dataset, config, result_queue):
...
def evaluate_pt_generation_multigpu(model, config, dataset, pedal_method='binary', max_samples=None):
    """
    Multi-GPU evaluation using process-based parallelism.
    Each GPU processes its own subset independently.
    """
    ...
    for gpu_id in range(num_gpus):
        ...
        if start_idx < num_samples:
            gpu_sample_indices.append(list(range(start_idx, end_idx)))
        else:
            gpu_sample_indices.append([])

    print(f"Samples per GPU: {[len(indices) for indices in gpu_sample_indices]}")
    ...
    # Launch processes
    processes = []
    for gpu_id, indices in enumerate(gpu_sample_indices):
        if not indices:
            continue
        p = mp.Process(target=process_gpu_worker, args=(gpu_id, indices, dataset, config, result_queue))
        p.start()
        processes.append(p)
...
def main():
    ...
    print("="*70)
    print("PT CORRECT Evaluation: Multi-GPU Generation")
    print("="*70)
    ...
    try:
        pred_features, target_features = evaluate_pt_generation_multigpu(
            pt_model, pt_config, test_dataset, args.pedal_method, args.max_samples
        )
        ...
        output_file = output_dir / f"pt_multigpu_results_{args.pedal_method}.json"
