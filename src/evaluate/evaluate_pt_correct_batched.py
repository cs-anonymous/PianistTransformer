"""
CORRECT PT Evaluation: Score -> Performance generation (BATCHED version)
Uses DataParallel and batched generation for high GPU utilization.
"""
...
def collate_fn(batch, config, max_length=512):
    ...
    for sample in batch:
        ...
        target_cont = sample['labels_continuous']

        # Skip if too long
        if len(pitch_ids) > max_length:
            continue

        score_input = create_score_input(pitch_ids, config)
        score_inputs.append(score_input)
        target_continuous.append(target_cont)
        lengths.append(len(pitch_ids))

    if not score_inputs:
        return None

    # Pad score inputs
    max_len = max(len(s) for s in score_inputs)
    padded_inputs = []
    attention_masks = []

    for score_input in score_inputs:
        pad_len = max_len - len(score_input)
        padded = score_input + [config.pad_token_id] * pad_len
        mask = [1] * len(score_input) + [0] * pad_len
...
def evaluate_pt_generation_batched(model, config, dataset, device, pedal_method='binary',
                                   max_samples=None, batch_size=8, num_workers=16):
    """
    BATCHED evaluation: Score -> Performance generation.
    Uses DataParallel for multi-GPU and batched generation.
    """
    ...
    print(f"Evaluating PT on {len(dataset)} samples (batched generation)...")
    print(f"Batch size: {batch_size}, Num workers: {num_workers}")

    for batch in tqdm(dataloader, desc="PT batched generation"):
        if batch is None:
            continue

        input_ids = batch['input_ids'].to(device)
        attention_mask = batch['attention_mask'].to(device)
        target_continuous_batch = batch['target_continuous']
        ...
        with torch.no_grad():
            ...
            generated_ids = actual_model.generate(
                ...
                max_new_tokens=512 * 8,  # Max tokens
                do_sample=False,
        ...
        for i in range(len(lengths)):
            try:
                pred_tokens = generated_ids[i].cpu().numpy().tolist()
                pred_cont = pt_tokens_to_continuous_features(pred_tokens, config)

                target_cont = np.array(target_continuous_batch[i])
...
def main():
    ...
    print("="*70)
    print("PT CORRECT Evaluation: Score -> Performance Generation (BATCHED)")
    print("="*70)
    ...
    try:
        pred_features, target_features = evaluate_pt_generation_batched(
        ...
        # Recompute overall with 5 metrics
        overall = {}
        ...
        output_file = output_dir / f"pt_correct_batched_results_{args.pedal_method}.json"
