...
def crop_near_white_border(path, padding=12, threshold=248):
    image = Image.open(path).convert("RGBA")
    pixels = np.asarray(image)
    rgb = pixels[:, :, :3]
    alpha = pixels[:, :, 3]
    content = (alpha > 0) & np.any(rgb < threshold, axis=2)
...
def main():
    ...
    ax.set_yticklabels([f"{v:g}" for v in pn_temps] + [f"{v:g}" for v in pp_temps], fontsize=10)
    ax.set_ylabel("Temperature", fontsize=11)
    ax.tick_params(length=0)
    ax.text(1.0, -0.82, "top-k", ha="center", va="center", fontsize=11)
    ax.text(4.0, -0.82, "top-p", ha="center", va="center", fontsize=11)
    ax.set_title("Sampling sensitivity (temperature >= 0.6)", fontsize=10.5, pad=16)
    ...
    pn_cbar.ax.set_title("PN", fontsize=9, pad=2)
    pp_cbar.ax.set_title("PP", fontsize=9, pad=2)
    pn_cbar.ax.tick_params(labelsize=9)
    pp_cbar.ax.tick_params(labelsize=9)

    args.out.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(args.out, dpi=300, bbox_inches="tight", pad_inches=0.02)
    crop_near_white_border(args.out)
