...
为避免一次性改动过大，当前仓库中的实现类名、脚本名、结果文件名仍保留 `Hybrid*` / `HN-*` 旧命名；本文档中讨论的新论文口径统一使用 `INR`，必要时会并列标注旧名。
...
当前已落地的第一版模型为 `HybridPianoT5Gemma`：保留 T5Gemma encoder-decoder backbone，新增 `HybridNoteEncoder` 和 `HybridContinuousDecoder`。按 `INR` 口径理解，它对应当前的第一版 `INR` backbone 实现。Pitch 只作为输入 embedding，不作为预测目标；decoder 使用 separate heads 分别预测 timing、velocity、pedal，并将 7 个连续字段 concat 后使用 sigmoid 限制在 `[0, 1]`。

需要特别说明：当前落盘数据已经是 `pianocore_hybrid_node_work_v2` schema，但当前训练代码仍主要服务第一阶段 EPR baseline；CSR 和完整对称 encoder/decoder 训练会在下一步实现。
...
在 Hybrid Note Representation 下，可以把当前工作统一看成一类 note-level conditional mapping，而不仅仅是单一的 `score -> performance`。
...
如果 EPR 成立，说明 Hybrid Note Representation 可以把“从乐谱到演奏”的表达映射建模为 note-level conditional prediction。  
而如果 CSR 也成立，则说明同一表示还能支持“从演奏回到规范乐谱”的逆向映射。两者合起来会让 HN 不只是一个 EPR trick，而是一套更通用的 note-level symbolic-performance interface。

#### 为什么 CSR 可能更适合 BERT

当前 `HN-BERT` 在 EPR 上的结果不理想，但这并不意味着 encoder-only 思路没有价值。相反，CSR 很可能正是更适合 BERT 的任务类型。原因是：
...
```text
HN-BERT 可能不适合 high-entropy 的 EPR，
...
这一版 Hybrid Note Representation 不再把 note 仅仅视为 “8 维 EPR 连续向量”，而是把它定义为一个统一的 note object。  
这个 object 同时支持：
...
- **Storage schema**: 磁盘上的 `*.node_a.json` / `*.node_hybrid.json` 字段，优先保持简单、兼容旧数据。
...
在当前 Hybrid Note schema 中，这三者对应关系是：
...
```json
{
  "schema": "pianocore_hybrid_node_work_v2",
  "meta": {
...
下面这部分是当前第一阶段 `EPR-only` 数据落地格式，用于已经训练完成的 T5/BERT 实验；它不是最终推荐的 HN 存储格式。
...
- [docs/figures/hybrid_backbone_structures.tex](/home/kaititech/EPR/PianistTransformer/docs/figures/hybrid_backbone_structures.tex)

该文件包含：

- `HN-T5` encoder-decoder 图
- `HN-GPT` decoder-only 图
- `HN-BERT` encoder-only 图
...
### 7.3.8 为什么 HN-BERT 看起来适合这个任务，但当前结果反而更差

先澄清一点：这里的 `HN-BERT` 并不是“只能输出一个向量”的分类器。当前实现中的 encoder-only backbone 会输出整条序列的 note-level hidden states，再由每个位置的 output head 回归对应的 performance 参数。对应代码位置：

- note encoder: [src/model/hybrid_pianoformer.py](/home/kaititech/EPR/PianistTransformer/src/model/hybrid_pianoformer.py:63)
- BERT backbone: [src/model/hybrid_pianoformer.py](/home/kaititech/EPR/PianistTransformer/src/model/hybrid_pianoformer.py:426)
- per-note continuous heads: [src/model/hybrid_pianoformer.py](/home/kaititech/EPR/PianistTransformer/src/model/hybrid_pianoformer.py:86)
- encoder-only wrapper: [src/model/hybrid_pianoformer.py](/home/kaititech/EPR/PianistTransformer/src/model/hybrid_pianoformer.py:476)

因此，`HN-BERT` 在能力上并非“不能做 seq2seq”，而是在做一种更窄的任务形式：
...
但“适合任务定义”不等于“当前配置下最容易学好”。截至 `2026-06-08` 的实测结果（见 [results/BACKBONE_EVALUATION_STATUS.md](/home/kaititech/EPR/PianistTransformer/results/BACKBONE_EVALUATION_STATUS.md)）里，`HN-BERT` 明显落后于两种 T5：
...
2. 把 pedal head 做得更强，甚至改成 mixture / discretized + regression hybrid head。
3. 使用 masked denoising pretraining，再做 EPR fine-tuning。
4. 在 loss 上降低 conditional mean 倾向，例如对 pedal 引入分段建模或更稳定的目标变换。

所以当前结论应该写成：

- `HN-BERT` 作为“最简 encoder-only aligned regression baseline”是成立的。
...
data/pianocore/PianoCoRe/refined/pianocore_a_hybrid_score_feature_update_summary.json
data/pianocore/PianoCoRe/refined/pianocore_a_hybrid_score_feature_update_details.jsonl
```
...
11. 将同一 Hybrid Note interface 迁移到 `CSR (Canonical Score Reconstruction)`，比较 `HN-T5 / HN-GPT / HN-BERT` 在逆向任务上的表现，重点验证 `HN-BERT` 是否在 CSR 上优于 EPR。
...
- 同一 HN 表示不只适用于 `score -> performance`，还应能自然扩展到 `performance -> canonical score (CSR)`。
- 如果 CSR 的目标熵确实低于 EPR，则 `HN-BERT` 有可能在 CSR 上显著优于其在 EPR 上的表现。

这将支持论文中的核心表述：Hybrid Note Representation 将 symbolic-performance 映射的表示层与 Transformer backbone 解耦，使 EPR 与 CSR 都可以作为 note-level conditional structured prediction 来研究，而不是被固定在 token-level language modeling 范式中。
