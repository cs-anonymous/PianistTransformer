...
RUN_ROOT="${RUN_ROOT:-results/rawlog_30_80_repro_4gpu/${STAMP}}"
...
# Preserve the exact non-chord rawlog setup that produced PP 29.60/87.95.
common = dict(base)
common.update(
    {
        ...
        "raw_timing_head_type": "distribution",
        "raw_timing_loss_lambda": 0.5,
        "tf_embedding_mask_keep_prob": 0.5,
        "tf_embedding_mask_score": True,
...
variants = {
    "cine": {"note_embedding_mode": "cine"},
    "sine": {"note_embedding_mode": "sine"},
    "slot5": {
...
for name, overrides in variants.items():
    ...
    cfg["run_name"] = f"rawlog_30_80_{name}"
...
keys = set().union(*(cfg.keys() for cfg in loaded.values()))
unexpected = []
for key in sorted(keys):
    values = {json.dumps(cfg.get(key), sort_keys=True) for cfg in loaded.values()}
    if len(values) > 1 and key not in allowed:
...
report = {
    ...
    "historical_pp_deterministic": {"ioi_wass": 29.601927118236983, "duration_wass": 87.9453947893381},
    "allowed_representation_differences": sorted(allowed),
    "common": {
        "epochs": common["num_train_epochs"],
        "batch_size": common["per_device_train_batch_size"],
        "gradient_accumulation": common["gradient_accumulation_steps"],
        "global_batch_size": common["global_batch_size"],
        "timing_head": common["raw_timing_head_type"],
        ...
        "musical_content_disabled": common["disable_musical_features"],
        "historical_input_continuous_dim": common["input_continuous_dim"],
    },
}
(config_dir / "config_diff_report.json").write_text(
...
launch_one() {
  local gpu="$1"
  local name="$2"
  local session="raw3080_${name}_${STAMP}"
