...
ADAPT_EPOCHS_LIST="${ADAPT_EPOCHS_LIST:-16}"
...
ROLLOUT_DIAG_ENABLE="${ROLLOUT_DIAG_ENABLE:-0}"
...
cleanup_training_pids() {
  local pids=()
  mapfile -t pids < <(pgrep -f "src/train/train_inr.py --config ${RUN_DIR}/config.json" || true)
  if [[ ${#pids[@]} -gt 0 ]]; then
...
run_rollout_diag() {
  local config="$1" checkpoint="$2" out_dir="$3" infer_gpu="$4"
  local cmd=(
  ...
  echo "[$(date '+%F %T')] rollout diag: checkpoint=${checkpoint}, gpu=${infer_gpu}, ks=${ROLLOUT_DIAG_KS}" | tee -a "${EVALUATE_LOG}"
...
python - "${RUN_CONFIG}" "${TRAIN_ROOT}" "${TF_LOG_ROOT}" "${BATCH_SIZE_PER_DEVICE}" "${GRADIENT_ACCUMULATION_STEPS}" "${GLOBAL_BATCH_SIZE}" "${BASE_RESUME_CHECKPOINT}" "${BASE_NUM_TRAIN_EPOCHS}" <<'PY'
import json, sys
path, train_root, tf_log_root, per_device_bs, grad_accum, global_bs, resume_checkpoint, base_epochs = sys.argv[1:9]
...
base_epochs = float(base_epochs)
cfg["num_train_epochs"] = base_epochs
...
cfg["max_train_epochs"] = base_epochs
...

{
  ...
  echo "SKIP_BASE_TRAIN ${SKIP_BASE_TRAIN}"
  echo "BASE_CHECKPOINT_OVERRIDE ${BASE_CHECKPOINT_OVERRIDE:-none}"
  echo "ADAPT_EPOCHS_LIST ${ADAPT_EPOCHS_LIST}"
  echo "BASE_NUM_TRAIN_EPOCHS ${BASE_NUM_TRAIN_EPOCHS}"
...
if [[ "${SKIP_BASE_TRAIN}" == "1" ]]; then
  [[ -n "${BASE_CHECKPOINT_OVERRIDE}" ]] || { echo "BASE_CHECKPOINT_OVERRIDE is required when SKIP_BASE_TRAIN=1" >&2; exit 1; }
  [[ -d "${BASE_CHECKPOINT_OVERRIDE}" ]] || { echo "BASE_CHECKPOINT_OVERRIDE does not exist: ${BASE_CHECKPOINT_OVERRIDE}" >&2; exit 1; }
  BASE_CHECKPOINT="${BASE_CHECKPOINT_OVERRIDE}"
  if [[ "${BASE_CHECKPOINT}" == */checkpoint-* ]]; then
    BASE_OUTPUT_DIR="$(dirname "${BASE_CHECKPOINT}")"
  else
    BASE_OUTPUT_DIR="${BASE_CHECKPOINT}"
  fi
  echo "[$(date '+%F %T')] base: skipped, using checkpoint=${BASE_CHECKPOINT}" | tee -a "${EVALUATE_LOG}"
else
...
adapt_epochs = float(epochs)
cfg["num_train_epochs"] = adapt_epochs
cfg["per_device_train_batch_size"] = int(per_device_bs)
cfg["per_device_eval_batch_size"] = int(per_device_bs)
cfg["gradient_accumulation_steps"] = int(grad_accum)
cfg["global_batch_size"] = int(global_bs)
...
cfg["early_stopping_patience"] = int(cfg.get("early_stopping_patience", 5))
cfg["early_stopping_threshold"] = float(cfg.get("early_stopping_threshold", 0.001))
cfg["max_train_epochs"] = adapt_epochs
cfg["eval_compute_wass"] = bool(cfg.get("eval_compute_wass", False))
cfg.setdefault("eval_dataloader_num_workers", cfg.get("dataloader_num_workers", 0))
cfg.setdefault("eval_dataloader_persistent_workers", False)
cfg.setdefault("eval_dataloader_prefetch_factor", cfg.get("dataloader_prefetch_factor", 2))
