...
cfg["use_prepared_cache"] = bool(cfg.get("use_prepared_cache", False))
cfg["use_prepared_sidecar"] = bool(cfg.get("use_prepared_sidecar", False))
cfg["prebuild_prepared_cache"] = bool(cfg.get("prebuild_prepared_cache", cfg["use_prepared_cache"] and not cfg["use_prepared_sidecar"]))
cfg["prebuild_eval_prepared_cache"] = bool(cfg.get("prebuild_eval_prepared_cache", False))
...
cfg["precompute_dataset_items"] = bool(cfg.get("precompute_dataset_items", False))
cfg["precompute_eval_dataset_items"] = bool(cfg.get("precompute_eval_dataset_items", False))
cfg["prebuild_prepared_cache"] = False
cfg["prebuild_eval_prepared_cache"] = False
cfg["save_steps"] = min(int(cfg.get("save_steps", 500)), 500)
cfg["eval_steps"] = min(int(cfg.get("eval_steps", 500)), 500)
cfg["save_total_limit"] = 2
cfg["load_best_model_at_end"] = True
