...
RUN_ROOT="${RUN_ROOT:-results/slot8_property_schedule_2gpu/${STAMP}}"
...
variants = {
    "slot8-correlated-perf-pad50": {
        "run_name": "slot8_correlated_perf_pad50",
        "prior_property_dropout_pattern": "correlated",
        "prior_property_dropout_replacement": "pad",
        "stable_dynamics_training": False,
        "stable_force_all_properties_visible": False,
    },
    "slot8-mixed-property-mask-stable": {
        "run_name": "slot8_mixed_property_mask_stable",
        "prior_property_dropout_pattern": "mixed",
        "prior_property_dropout_replacement": "mask",
        # Stable noise occupies 30% all-visible tokens. On the remaining 70%,
        # these probabilities yield 50/25/25 overall in expectation.
        "prior_property_visible_prob": 2.0 / 7.0,
        "prior_property_all_dropout_prob": 5.0 / 14.0,
        "stable_dynamics_training": True,
        "stable_apply_prob": 0.30,
        "stable_channels": ["ioi", "duration"],
        "stable_noise_modes": stable_noise_modes,
        "stable_force_all_properties_visible": True,
    },
}
...
launch_one() {
  local gpus="$1"
  local name="$2"
  local session="slot8sched_${name//[^A-Za-z0-9_]/_}_${STAMP}"
...
launch_one "0,1" "slot8-correlated-perf-pad50"
launch_one "2,3" "slot8-mixed-property-mask-stable"
