...
RUN_ROOT="${RUN_ROOT:-results/inr_epr_pipeline/asap_rebuilt_3baseline_${STAMP}}"
...
for name, overrides in experiments.items():
    ...
    cfg["run_name"] = f"asap_rebuilt_{name}"
...
nohup bash "${RUN_ROOT}/run_one.sh" 0 full_musical_gated_zero_init "${RUN_ROOT}" "${ROOT_DIR}" > "${RUN_ROOT}/full_musical_gated_zero_init.nohup.log" 2>&1 &
echo $! > "${RUN_ROOT}/full_musical_gated_zero_init.pid"
nohup bash "${RUN_ROOT}/run_one.sh" 1 no_musical "${RUN_ROOT}" "${ROOT_DIR}" > "${RUN_ROOT}/no_musical.nohup.log" 2>&1 &
echo $! > "${RUN_ROOT}/no_musical.pid"
nohup bash "${RUN_ROOT}/run_one.sh" 2 full_musical_mlp "${RUN_ROOT}" "${ROOT_DIR}" > "${RUN_ROOT}/full_musical_mlp.nohup.log" 2>&1 &
echo $! > "${RUN_ROOT}/full_musical_mlp.pid"
